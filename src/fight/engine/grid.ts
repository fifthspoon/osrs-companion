...
// Chebyshev distance from a tile to a square block (0 if inside it).
...
// One tile step from `from` toward `target` (diagonals allowed, OSRS-style).
...
// Pixel (canvas space) -> tile, or null if outside the arena.
